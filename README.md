...
- 把 CF 免费 AI 模型转成 OpenAI `/v1/chat/completions` 格式，Cursor、LobeChat、NextChat 等客户端直接接入
- 支持 `/v1/embeddings` 和 Anthropic `/v1/messages`
- 绑定多个 CF 账号，自动负载均衡，某个账号额度用完或挂了自动换下一个
- 可视化管理面板：查看用量、管理账号/密钥、自定义模型映射
- API Key 鉴权（不配密钥就是公开的）
...
| 客户端 | Base URL | 注意 |
|--------|----------|------|
| Cursor | `https://域名/v1` | Settings → Models → OpenAI API |
| NextChat | `https://域名` | 不要带 `/v1` |
| LobeChat | `https://域名/v1` | |
